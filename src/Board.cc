...
bool Board::put_on_board(Row_Col pos, Tile tile, bool is_player) {
  ...
  tile.rect.w = tile.rect.h = constants::kSquarePixelSize;
  ...
  if (is_player) {
    place_highlight(tile.rect, is_player);
  }
  return true;
}
...
void Board::play_placed_tiles(bool is_player) {
  ...
  if (is_player) {
    for (const auto &tile : recently_placed_) {
      SDL_RenderCopy(renderer_, tile.texture, &kTrimmedTile, &tile.rect);
    }
  } else {
    ...
    for (const auto &tile : computer_highlighted_tiles_) {
      place_highlight(tile.rect, false);
    }
  }
  SDL_SetRenderTarget(renderer_, nullptr);
  for(auto& blank: blanks_){
    blank.rect.w = constants::kTileWidth;
    blank.rect.h = constants::kTileHeight;
  ...
  for(auto& tile: recently_placed_){
    ...
    tile.rect.w = constants::kTileWidth;
    tile.rect.h = constants::kTileHeight;
